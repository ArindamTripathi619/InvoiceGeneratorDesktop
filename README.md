...
## 🐳 Docker (quick test on any OS including Windows)

If you just want to run and test the frontend quickly (without building a native Tauri bundle), you can use Docker. This is useful for Windows users with Docker Desktop — start the app and open it in your browser with a single command.

Prerequisites:

Quick run (build + serve production static files):
...
Please read these limitations before relying on the Docker setup for testing:

- Native Tauri APIs are not available inside the container: any code that imports `@tauri-apps/api` and expects native filesystem access, BaseDirectory paths, or Tauri dialogs will not behave the same as the native Tauri app. The container serves only the web frontend.
- PDF saving in the desktop app uses Tauri's `writeBinaryFile` to save to the AppData `generated/` folder and `save()` for a native save dialog. Inside Docker (served in a browser) jsPDF will still create a client-side download, but the files will not be written to the host's AppData `generated/` path by the Tauri backend.
- Native dialogs (message boxes, save/open dialogs) implemented via Tauri will not appear in the browser; the web app may fall back to browser dialog behavior or fail where code expects Tauri responses.
- The Docker container does not build or run the Rust backend. Building native installers (.msi/.exe) requires running `npm run tauri:build` on Windows (or a Windows CI runner). Docker here is for fast frontend testing only.
- If you need to exercise Tauri-specific flows (native filesystem writes, platform permissions, or shell open), test on a Windows machine with Tauri installed or set up a CI workflow using a Windows runner.

If you want, I can add a small GitHub Actions workflow that builds the Tauri app on a Windows runner and uploads the installer artifacts — tell me and I'll add it.
